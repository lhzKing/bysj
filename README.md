...
- ✅ **新增「聚合关系」前端工作台** `frontend/src/features/trace/views/TraceAggregationWorkbench.vue`：左侧菜单新条目（producer/warehouse/logistics/user 可见，仅 `trace:view`），按 `parent_code` 分组展示所有 active 聚合，支持 relation_type chips（全部/CARTON/PALLET）+ 父码/子码/创建人模糊搜，每行有「解除」按钮（带 confirm + 写 UNPACK/UNPALLETIZE 日志）；顶部「新建装箱 / 新建装托」打开 `AggregationBindDialog.vue`（关系类型 chips + 父码/子码 mono input + 摄像头扫码按钮 + 备注），写入权限不足时（如 USER 角色）按钮自动隐藏
