...
> 调用 `POST /api/admin/seed-master-data` 后会**额外**种入 4 个二级演示账号（`producer2`, `warehouse2`, `logistics2`, `dealer1`），密码同主账号（如 `producer2 / producer123456`），用于演示「同一角色多账号 + 不同节点绑定」场景。蓝本见 [`DemoMasterDataBlueprint.java`](backend/src/main/java/com/example/trace/service/impl/support/DemoMasterDataBlueprint.java)。
...
│   ├── pom.xml
│   ├── sql/                       建表 + 22 个 migrate 迁移 + 演示数据
│   │   ├── init_schema.sql        全新建库脚本（含 6 角色 + 24 权限）
│   │   ├── schema_consolidated.sql 等价 mysqldump，docker mysql-init 用
│   │   ├── migrate_v8 ~ v22.sql   逐步加表/字段：幂等表 / 节点 / 任务 / 聚合 / 异常 / seed-master 权限 ...
│   │   └── sample_data_full.sql   500 条带 hash+RSA 的可验证示例
│   └── src/main/
│       ├── java/com/example/trace/   见下文「后端架构」章
│       │   └── service/impl/support/
│       │       ├── DemoMasterDataBlueprint.java   主数据蓝本常量（8 user / 19 node / 15 part / 16 binding）
│       │       ├── DemoChainBuilder.java          单 trace 5-7 步 lifecycle 链 + 签名
│       │       ├── DemoFlowTaskFactory.java       60 任务 + scan，状态分布精确
│       │       ├── DemoAggregationFactory.java    24 箱 + 6 托盘 父子聚合
│       │       └── TraceBatchCommitter.java       T-P1-01 两阶段提交器
│       └── resources/
...
| GET  | `/api/trace-aggregations[?relation_type=]` | `trace:view`（前端聚合工作台一次性加载 active 列表） |
...
| GET | `/api/users/me/trace-nodes` | `trace:view` |
| GET | `/api/users/{id}/trace-nodes` | `user:view` |
| PUT | `/api/users/{id}/trace-nodes` | `user:manage` |
...
| POST | `/api/admin/seed-master-data` | `trace:data:seed-master` |
...
| v20 | 异常工作流 |
| v21 | part.enabled |
| **v22** | **trace:data:seed-master 权限**（POST /api/admin/seed-master-data；授给 SUPER_ADMIN） |
...
### 9.6 演示数据接口（仅 dev/test，prod 默认禁用）

干净库（仅 RBAC + superadmin）→ 完整 demo 数据，**两步 HTTP 接口搞定，不再依赖 Python 脚本**：
...
# 2. 种主数据（demo 用户 / 节点 / SPU / 用户-节点绑定，幂等）
#    新增 8 个 demo 账号（producer/warehouse/logistics/user + 二级账号 producer2 等）
#    19 个 trace_node（FACTORY×5 / WAREHOUSE×7 / LOGISTICS×5 / CUSTOMER×2，覆盖 14 省）
#    15 个 base_part_spec（5 大类 × 3 款）
#    16 条 trace_user_node_binding
curl -X POST "http://localhost:8080/api/admin/seed-master-data" \
  -H "Authorization: Bearer $TOKEN"

# 3. 一站式灌业务流水（8 张表，hash + RSA 真签名，前端 /verify 100% 通过）
#    25 batches / 500 trace_code / ~3000 lifecycle_log / 500 snapshot /
#    60 flow_task（5 状态精确分布）/ ~600 scan / ~325 aggregation
curl -X POST "http://localhost:8080/api/admin/generate-sample-data?count=500" \
  -H "Authorization: Bearer $TOKEN"

# 4. 验任意码（应返回 valid=true，N/N hash + N/N signature 全通过）
curl "http://localhost:8080/api/traces/<code>/verify" \
  -H "Authorization: Bearer $TOKEN"

# 5. 清空业务流水（保留主数据，二次确认）
#    按 FK 顺序清 8 张表：flow_task_scan → flow_task → aggregation →
#    scan_idempotency → lifecycle_log → snapshot → trace_code → assign_batch
curl -X DELETE "http://localhost:8080/api/admin/clear-trace-data?confirm=DELETE_TRACE_DATA" \
  -H "Authorization: Bearer $TOKEN"
```

权限：seed-master 需要 `trace:data:seed-master`（SUPER_ADMIN）；generate 需要 `trace:data:generate`；clear 需要 `trace:data:clear`。

prod 默认禁用：`TRACE_DEMO_DATA_ENABLED=true` 才打开。`docker-compose.yml` 已把这个变量透传到容器；改 .env 后需 `docker compose up -d backend` 让容器重读环境变量（restart 不重新读 .env）。

离线兜底 `scripts/seed_extended_data.py` 保留，跑前需要 `pip install pymysql cryptography` + 拿到 RSA 私钥，正常演示不需要。
...
- `POST /api/admin/generate-sample-data` 仅覆盖 lifecycle 流水、v10–v21 模块全部漏写 + 演示依赖 Python 脚本 → 新增 `POST /api/admin/seed-master-data`（幂等种主数据）+ generate-sample-data 一站式产 8 张业务表（batch / trace_code / lifecycle / snapshot / flow_task / scan / aggregation），真签真链，HTTP 两步搞定不依赖 Python（2026-05-27）；
- 演示数据中 EXCEPTION_OPEN 没保存 exception_restore_* 字段 → 用户后续点「解除冻结」会被 TraceTransitionPolicy 拒绝 → DemoChainBuilder 同步对齐 TraceScanTransactionService 真实扫码路径（2026-05-27）；
- 节点绑定 PUT 端点不校验目标角色，USER 也能被绑 → 后端 NODE_BINDABLE_ROLES 白名单只允许 PRODUCER/WAREHOUSE/LOGISTICS/ADMIN/SUPER_ADMIN；前端 UserTable 对 USER 隐藏按钮，对 ADMIN/SUPER_ADMIN 弹 warning banner 提示"仅救场使用"（2026-05-27）；
- 用户管理页缺少 trace_node 绑定 UI（后端 3 个接口就绪但前端没暴露）→ 新增 UserNodeBindingDialog 多选 + 默认节点星标 + 按节点类型分组（2026-05-27）；
- 重启后端后旧 token 失效但前端 localStorage 仍有 token+user，ScanHub / TraceScanLanding 等"mount 时不调 API"的页面感知不到失效 → MainLayout onMounted 静默 ping `/api/auth/userinfo` 触发 401 → 自动跳登录（2026-05-27）；
- CSP 只白名单了 webapi.amap.com / restapi.amap.com，AMap 内部 watchSize 用 `javascript:` URL 监听 + 拉 vdata.amap.com 元数据被拦 → 移动端只能看到 marker 看不到瓦片 → CSP 放行 `*.amap.com` + 加 `'unsafe-inline'`（2026-05-27）；
- ScanHub 容器 `display:none` 时 `new AMap.Map` 测出 0×0 视口 → DemoChainBuilder 调整 init 顺序 + `requestAnimationFrame(map.resize)` 兜底（2026-05-27）；
- ScanHub 页缺 `max-width + padding` 对齐基线，内容紧贴左上角 → 补齐与 UserList / TraceList 同款约束（2026-05-27）；
- docker-compose.yml backend environment 段没透传 `TRACE_DEMO_DATA_ENABLED` → .env 改了没用 → 显式声明并兼顾 `TRACE_DEMO_DATA_MAX_GENERATE_COUNT`，default false（2026-05-27）。
...
**箱码 / 托盘码聚合 UI（FR-06）+ Demo 聚合数据补链：**

- ✅ **新增「聚合关系」前端工作台** `frontend/src/features/trace/views/TraceAggregationWorkbench.vue`：左侧菜单新条目（producer/warehouse/logistics + admin/superadmin 可见；纯只读 **user 角色不可见**——菜单/路由门槛用聚合写权限集 `PERMISSIONS.TRACE.AGGREGATION_ACCESS`＝`[trace:task:scan, trace:create, trace:scan, trace:outbound, trace:transfer]`，与后端 bind/release 端点 `@RequirePermission` 一致，而非 `trace:view`），按 `parent_code` 分组展示所有 active 聚合，支持 relation_type chips（全部/CARTON/PALLET）+ 父码/子码/创建人模糊搜，每行有「解除」按钮（带 confirm + 写 UNPACK/UNPALLETIZE 日志）；顶部「新建装箱 / 新建装托」打开 `AggregationBindDialog.vue`（关系类型 chips + 父码/子码 mono input + 摄像头扫码按钮 + 备注），写入权限不足时按钮自动隐藏
- ✅ **新增 `GET /api/trace-aggregations[?relation_type=]` 端点**（`trace:view`）：现有 5 个 GET/POST 端点都是按单父码 / 单子码查询，无法一次性显示工作台需要的 active 全量；新加的第 6 个端点不修改其它 5 个的语义和返回结构，只增量补「列出全部 active」能力。前端按 relation_type 调用后再做客户端模糊搜
- ✅ **修复 `DemoAggregationFactory` 不写 PACK/PALLETIZE 上链事件**：之前 325 条聚合直接 INSERT trace_aggregation，绕过 `TraceLogFactory`，导致 demo 库里没有任何 PACK/PALLETIZE 日志、`/verify` 看不出聚合关系。重写后：每个 CARTON binding 给单品码追加 1 条 PACK 日志；每个 (PALLET, carton) binding 展开为「该 carton 下所有单品码各 1 条 PALLETIZE」（对齐真实 `TraceAggregationServiceImpl.resolveMutableChildSnapshots` 语义）。日志走 `TraceLogFactory` 真签真链，链 prev_hash 接续 trace 原 lastHash，snapshot.lastHash/lastEventTime 同步推进
- ✅ **demo 父码前缀切到 `CARTON-EXT-` / `PALLET-EXT-`**（之前 `CTN-EXT-` / `AGG-EXT-PALLET-`）：让父码能通过 `TraceAggregationServiceImpl.isParentAggregationCode` 检查，未来从 demo 数据走真实 bind/release 路径也能直接通过验证；不影响 schema，下次 `clear-trace-data` + `generate-sample-data` 生效
- ✅ **测试覆盖**：新增 `TraceAggregationWorkbench.smoke.test.js`（13 用例）+ `TraceAggregationWorkbench.contract.test.js`（6 用例）覆盖列表/分组/筛选/搜索/绑定 dialog 路由/解除流程/权限门控；`DemoAggregationFactoryTest` 扩 4 个用例锁住 PACK/PALLETIZE 数量、hash 链可重算、缺失 chain 容错。前端 350 测试 / 后端 422 测试全部通过

### 2026-05-27

**演示数据基础设施重构 + 节点绑定 UI + 多个修复：**

- ✅ **`POST /api/admin/seed-master-data` 新增**（幂等）：种 8 个 demo 账号 / 19 个 trace_node / 15 个 SPU / 16 条 user-node 绑定，密码现场 BCrypt 哈希；新增 `trace:data:seed-master` 权限（migrate_v22）授给 SUPER_ADMIN
- ✅ **`POST /api/admin/generate-sample-data` 重写**为一站式：单次调用覆盖 8 张业务表（batch / trace_code / lifecycle_log / snapshot / flow_task / scan / aggregation），每条 log 走 `TraceLogFactory` 真签真链，前端 `/verify` 100% 通过；保持 T-P1-01 两阶段提交（stage 1 in-memory + stage 2 chunked REQUIRES_NEW）
- ✅ **`DELETE /api/admin/clear-trace-data` 扩展**：按 FK 顺序清 8 张业务表（`flow_task_scan → flow_task → aggregation → scan_idempotency → lifecycle_log → snapshot → trace_code → assign_batch`），保留主数据
- ✅ **不再依赖 `scripts/seed_extended_data.py`**：干净库 → 完整 demo 数据现在 2 次 HTTP 调用搞定；Python 脚本保留作离线兜底
- ✅ **用户管理页新增「节点绑定」对话框**：`UserNodeBindingDialog.vue` 多选 + 单选默认节点（星标）+ 按 nodeType 分组（工厂/仓储/物流/客户/服务），底部实时显示已选数；ADMIN/SUPER_ADMIN 行显示黄色 warning banner（"仅救场使用"）
- ✅ **节点绑定后端角色白名单**：`NODE_BINDABLE_ROLES = {PRODUCER, WAREHOUSE, LOGISTICS, ADMIN, SUPER_ADMIN}`，USER 被拒（PARAM_ERROR）；前端 UserTable 对 USER 行隐藏「节点绑定」按钮
- ✅ **修 ScanHub / TraceScanLanding 不跳登录**：MainLayout onMounted 静默 ping `/api/auth/userinfo`，无效 token 立即被 request.js 拦截器跳登录，不必等用户操作触发 API
- ✅ **修 AMap 移动端瓦片不渲染**：CSP `script-src` 加 `'unsafe-inline'`（AMap watchSize 用 `javascript:` URL）+ 把 `*.amap.com` 整域放行（含 vdata.amap.com 元数据 JSONP）；DemoChainBuilder 调整 init 顺序避免 `new AMap.Map()` 时容器 `display:none` 测出 0×0
- ✅ **修 DemoChainBuilder EXCEPTION_OPEN 不保存恢复字段**：演示数据中异常 trace 之前无法 EXCEPTION_CLOSE（"缺少可恢复的原状态"），现已对齐 `TraceScanTransactionService` 真实扫码路径，snapshot 三元组同步写入 `exception_restore_*`
- ✅ **修 docker-compose 未透传 `TRACE_DEMO_DATA_ENABLED`**：environment 段显式声明 + 默认 false；prod 启用 demo 接口走 `.env` + `docker compose up -d backend`
- ✅ **修 ScanHub 内容紧贴左上角**：补齐 `max-width: 1280px + padding: 24px 24px 48px` 与 UserList/TraceList 同款基线
- ✅ **修 WebMvcSecurityPathContractTest 长期红线**：补 `/api/public/**` 到 excludePatterns 断言
