        ...
        <section class="premium-card rounded-[36px] p-6 lg:p-8" data-test="flow-task-detail">
          <template v-if="selectedTask">
            ...
            <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
              ...
              <aside class="rounded-[28px] border border-slate-200 bg-white/80 p-5"><p class="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Operator Guide</p><h3 class="mt-2 text-lg font-black text-slate-900">一线操作提示</h3><ul class="mt-5 space-y-3 text-sm font-bold text-slate-500"><li class="flex gap-3"><Truck class="mt-0.5 h-5 w-5 flex-shrink-0 text-indigo-500" />出库任务会自动写出库事件。</li><li class="flex gap-3"><Warehouse class="mt-0.5 h-5 w-5 flex-shrink-0 text-emerald-500" />目标节点扫码会自动写接收入库事件。</li><li class="flex gap-3"><AlertTriangle class="mt-0.5 h-5 w-5 flex-shrink-0 text-amber-500" />重复扫码提示“已扫”，不会重复计数。</li></ul></aside>
            </div>
          </template>
