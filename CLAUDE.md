...
| `POST /api/admin/generate-sample-data` 只覆盖 lifecycle 流水、不覆盖 v10–v21 模块；演示数据依赖 `scripts/seed_extended_data.py` Python 脚本 | 已重构：新增 `POST /api/admin/seed-master-data`（幂等种 demo 用户/节点/SPU/绑定）+ `generate-sample-data` 一站式产 8 张业务流水表（batch / trace_code / lifecycle / snapshot / flow_task / scan / aggregation），全部走 `TraceLogFactory` 真签名；`clearTraceData` 按 FK 顺序清 8 张表保留主数据。两步 HTTP 调用即可灌完整 demo，不再依赖 Python。证据：`backend/src/main/java/com/example/trace/service/impl/TraceDemoDataServiceImpl.java`、`backend/src/main/java/com/example/trace/service/impl/TraceMasterDataSeedServiceImpl.java`、`backend/src/main/java/com/example/trace/service/impl/support/Demo*.java`、`backend/sql/migrate_v22_demo_seed_master_permission.sql` |
| `DemoAggregationFactory` 直接 INSERT trace_aggregation 绕过 `TraceLogFactory`，导致演示数据 325 条聚合关系在 `trace_lifecycle_log` 里没有对应的 PACK/PALLETIZE 上链事件 | 已重构：工厂改为先在内存里 build 30 个 parent + 325 子行的同时，按真实 `TraceAggregationServiceImpl` 语义对每个子 trace 追加 PACK 日志（每条 CARTON binding 一条）+ PALLETIZE 日志（每个 (PALLET, carton) 展开为该 carton 下所有单品码各一条），全部走 `TraceLogFactory` 真签真链；snapshot.lastHash 同步推进。父码前缀改为 `CARTON-EXT-` / `PALLET-EXT-`，能直接通过 `TraceAggregationServiceImpl.isParentAggregationCode` 检查。证据：`backend/src/main/java/com/example/trace/service/impl/support/DemoAggregationFactory.java`、`backend/src/main/java/com/example/trace/service/impl/TraceDemoDataServiceImpl.java`（PendingTraceUnit + 重排序使聚合 mutate 在 unit 冻结前）、`backend/src/test/java/com/example/trace/service/impl/support/DemoAggregationFactoryTest.java`（hash 可重算 / 数量 / 链连续性） |
| 前端缺「箱码 / 托盘码聚合」用户操作入口，FR-06 形同虚设 | 已落地：`frontend/src/features/trace/views/TraceAggregationWorkbench.vue` 按 parent_code 分组展示所有 active 聚合（PRODUCER/WAREHOUSE/LOGISTICS + ADMIN/SUPER_ADMIN 可见；纯只读 USER 不可见——菜单/路由门槛 = `PERMISSIONS.TRACE.AGGREGATION_ACCESS` 聚合写权限集，**不是** `trace:view`，与后端 bind/release 端点 `@RequirePermission` 一致），filter chips（全部/CARTON/PALLET）+ 父码/子码模糊搜；顶部「新建装箱 / 新建装托」打开 `AggregationBindDialog.vue`，复用 QRScanner 控件；每行「解除」走 confirm + `POST /{id}/release`。后端补 `GET /api/trace-aggregations[?relation_type=]`（`trace:view`）一次性拉全量；既有 5 个端点语义不变。证据：`frontend/src/features/trace/views/TraceAggregationWorkbench.vue`、`frontend/src/features/trace/components/AggregationBindDialog.vue`、`frontend/src/features/trace/api/aggregation.js`、`frontend/src/shared/constants/permissions.js#TRACE_AGGREGATION_ACCESS`、`backend/src/main/java/com/example/trace/controller/TraceAggregationController.java`、`backend/src/main/java/com/example/trace/mapper/TraceAggregationMapper.java#selectAllActive` |
